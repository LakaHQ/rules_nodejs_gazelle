		...
		if len(kindSources) > 0 {
			...
			if aggregateModule {
				...
				for i := range moduleRules {
					moduleRule := moduleRules[i]
					if appendTS {
						moduleRule.SetAttr("tsconfig", "//node/packages:tsconfig.json")
						moduleRule.SetAttr("validate", false)
					}
				...
				for i := range singletonRules {
					...
					singletonRule := singletonRules[i]
					if appendTS {
						singletonRule.SetAttr("tsconfig", "//node/packages:tsconfig.json")
						singletonRule.SetAttr("validate", false)
					}

					generatedRules = append(generatedRules, singletonRule)
